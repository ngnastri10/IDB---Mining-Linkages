* ==========================================================================
* Clean SCM Guia de Utilizacao Autorizada: one row per authorized
* extraction record (a process can have several over time).
*
* Input:
*   Data/SCM/Guia/Guia_de_Utilizacao_Autorizada.csv
*
* Output:
*   Working/SCM/scm_guia_record.dta
*
...
*		Section 3: Translate substance into english
*		Section 4: Label variables and save
* ==========================================================================
...
import delimited using "`SCM_RAW'/Guia/Guia_de_Utilizacao_Autorizada.csv", ///
    clear varnames(1) encoding("windows-1252")

rename (titular localidade guia substancia quantidademinerio datapublicacao prazo indicaprorrogacao) ///
       (holder_name municipality_text guide_number substance quantity_text date_published_text validity_period_text is_renewal_text)
...
processo here looks like "003.668/1961" - periods in the number part. Split on "/", strip the periods, destring both sides.
*/
...
replace process_number_str = subinstr(process_number_str, ".", "", .)
...
* Sim/Nao -> 1/0
generate byte is_renewal = (is_renewal_text == "Sim")
drop is_renewal_text

* quantity_text and validity_period_text are both messy free text
* (mixed units, inconsistent formats) - left as raw text on purpose,
* not parsed into numbers here.
...
Same dictionary as SIGMINE/CFEM, but substance here is messier - mixed
case, and some values have extra text tacked on like "(Minerio de)" or
"(uso Industrial)" that won't match the dictionary exactly. Uppercasing
first catches the plain case-only mismatches; the parenthetical ones
will still come through untranslated.
*/

generate substance_original = trim(upper(substance))
...
local newnames process_number process_year holder_name municipality_text ///
    guide_number substance substance_original quantity_text date_published ///
    validity_period_text is_renewal
...
foreach v of local newnames {
    ...
    local lbl = cond("`v'" == "process_number", "Process number, without the year", ///
                ...
                cond("`v'" == "guide_number", "Guia number - identifies this specific authorization", ///
                cond("`v'" == "substance", "Mineral substance authorized for extraction", ///
                cond("`v'" == "substance_original", "Mineral substance, original Portuguese text", ///
                cond("`v'" == "quantity_text", "Authorized quantity, raw text - mixed units, not parsed", ///
                cond("`v'" == "date_published", "Date this guia was published", ///
                cond("`v'" == "validity_period_text", "Validity period, raw text - inconsistent format, not parsed", ///
                cond("`v'" == "is_renewal", "1 if this record is a renewal/extension of a prior guia", ///
                "Error `v'")))))))))))
...
save "`SCM_WORKING'/scm_guia_record.dta", replace

di as result "Done - saved `=_N' guia records to `SCM_WORKING'/scm_guia_record.dta"
